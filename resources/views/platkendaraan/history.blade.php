                    ...
                    <table class="min-w-full divide-y divide-gray-200 rounded-xl overflow-hidden">
                        <thead>
                            <tr class="bg-orange-500">
                                <th class="px-4 py-3 text-left text-xs font-bold text-white uppercase">Tanggal Perbaikan
                                </th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse($riwayats as $riwayat)
                                <tr class="hover:bg-orange-50 transition">
                                    <td class="px-4 py-3 text-black">
                                        {{ \Carbon\Carbon::parse($riwayat->tanggal_perbaikan)->format('d/m/Y') }}</td>
                                    <td class="px-4 py-3 text-gray-800">{{ $riwayat->deskripsi_perbaikan }}</td>
                                    <td class="px-4 py-3">
                                        @if ($riwayat->dokumentasi_perbaikan)
                                            <a href="{{ asset('storage/' . $riwayat->dokumentasi_perbaikan) }}"
                                                target="_blank">
                                                <img src="{{ asset('storage/' . $riwayat->dokumentasi_perbaikan) }}"
                                                    alt="Dokumentasi"
                                                    class="h-16 rounded-lg border-2 border-orange-500 shadow">
                                            </a>
                                        @else
                                    ...
                                    <td class="px-4 py-3 text-black">
                                        {{ \Carbon\Carbon::parse($riwayat->next_service)->format('d/m/Y') }}</td>
                                    <td class="px-4 py-3 text-black">
                                        @if ($riwayat->mekanik)
                                            <a href="{{ route('konsumen.mekanik-profile', $riwayat->mekanik->id_mekanik ?? $riwayat->mekanik->id) }}"
                                                class="text-orange-600 hover:text-orange-800 font-medium">
                                                {{ $riwayat->mekanik->user->name ?? '-' }}
                                            </a>
                                        @else
                                            <span class="text-gray-400">-</span>
                                        @endif
                            ...
                            @empty
                                <tr>
                                    <td colspan="5"
                                        class="px-4 py-8 text-center text-gray-500 bg-orange-50 rounded-xl">
                                        <div class="flex flex-col items-center">
                                            <svg class="h-16 w-16 text-orange-200 mb-4" fill="none"
                                            ...
                                            <span class="text-lg font-bold text-black mb-2">Belum ada riwayat
                                                perbaikan.</span>
                                            <span class="text-base text-gray-500">Kendaraan ini belum pernah melakukan
                                                perbaikan.</span>
                                        </div>
                                    </td>
                                </tr>
