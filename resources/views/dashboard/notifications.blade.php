<div class="mb-6">
    <button onclick="document.getElementById('notifPanel').classList.toggle('hidden')"
        ...
        🔔 Lihat Notifikasi ({{ auth()->user()->unreadNotifications->count() }})
    </button>
            ...
            <ul class="divide-y divide-gray-200 max-h-80 overflow-y-auto" id="notifList">
                ...
                @forelse($unreadNotifs as $notif)
                    <li class="py-2 flex justify-between items-center" id="notif-{{ $notif->id }}">
                        ...
                        <button onclick="markNotifRead('{{ $notif->id }}')" class="ml-4 px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200">
                            Sudah Dibaca
                        </button>
                    </li>
                @empty
    ...
    fetch('/notifications/mark-read/' + id, {
        ...
        if (res.ok) {
            ...
            if (document.querySelectorAll('#notifList li').length === 0) {
                document.getElementById('notifList').innerHTML = '<li class="py-2 text-gray-400 text-center">Belum ada notifikasi baru.</li>';
            }
        }
    });
}
